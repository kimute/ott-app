...
import { getPopular, getRecommend, getVideos, IGetMovie} from "../api";
...
function RecommendSlider(){
    ...
    const routeMovieMatch = useRouteMatch<{movieId: string}>("/mylist/recommend/:movieId");
    const { data:popData, isLoading:popLoading } = useQuery<IGetMovie>(["mylist", "popular"], getPopular);
    const num = popData?.results[0].id
    const { data, isLoading } = useQuery<IGetMovie>(["mylist", "recommend"], ()=>getRecommend(num || 634649));
    ...
    const onMovieClick = (movieId: number) =>{
        history.push(`/mylist/recommend/${movieId}`);
    ...
    const goBackClick = () => history.push("/mylist");
