...
import { getTvtoprate, getTvVideos, getVideos, ITvpop} from "../api";
...
const offset = 6;
function TvToprateSlider(){
    const history = useHistory()
    const { scrollY } = useViewportScroll();
    const routeMovieMatch = useRouteMatch<{tvId: string}>("/tv/onair/toprate/:tvId");
    const { data, isLoading } = useQuery<ITvpop>(["tv", "toprate"], getTvtoprate);
    ...
    const oonTvClick = (tvId: number) =>{
        history.push(`/tv/onair/toprate/${tvId}`);
    ...
    const clickedItem = routeMovieMatch?.params.tvId && data?.results.find(
        movie => String(movie.id) === routeMovieMatch.params.tvId);
        ...
        <Wrapper>
            {isLoading ? 
                ...
                (<>
                    ...
                    <MiniTitle>Tv Top rated {'>'} </MiniTitle>
                    ...
                    <AnimatePresence>
                        ...
                        <InfoMovie
                            ...
                            layoutId={routeMovieMatch.params.tvId}>
                            <motion.svg
                                onClick={goBackClick}
...
export default TvToprateSlider;
