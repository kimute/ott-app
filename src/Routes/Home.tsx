...
import { getMovie, getVideos, IGetMovie} from "../api";
import {imagePath} from "../utils";
import PopularSlider from "../Components/Popular";
import TopRatedSlider from "../Components/UpcomSlider";
...
function Home(){
    ...
    const routeMovieMatch = useRouteMatch<{movieId: string}>("/home/movies/:movieId");
    const { scrollY } = useViewportScroll();

    const { data, isLoading } = useQuery<IGetMovie>(["movie", "nowPlaying"], getMovie);

    data?.results.map(
        i=> getVideos(i.id).then(
    ...
    const onMovieClick = (movieId: number) =>{
        history.push(`/home/movies/${movieId}`)
            ...
            {isLoading ? 
                ...
                (<>
                    <Banner 
                        ...
                        onClick={()=>onMovieClick(data?.results[0].id || 0)}
                    ...
                    <MiniTitle>Now Playing {'>'} </MiniTitle>
