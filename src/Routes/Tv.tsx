...
import { getTvs, getTvVideos, IGetTv} from "../api";
import {imagePath} from "../utils";
import TvPopular from "../Components/TvPopular";
import TvToprateSlider from "../Components/TvToprated";
...
const BannerVideo = styled(motion.div)`
    margin-top: 10px;
    width: 120px;
    height: 40px;
    border-radius:5px;
...
const InfoTv = styled(motion.div)`
    position: absolute; 
    width: 800px; 
    height: 700px;
...
//paging: movies want to show
const offset = 6;
function Tv(){
    const history = useHistory()
    const routeTvMatch = useRouteMatch<{tvId: string}>("/tv/onair/:tvId");
    const { scrollY } = useViewportScroll();

    const { data, isLoading } = useQuery<IGetTv>(["tv", "nowPlaying"], getTvs);

    data?.results.map(
        i=> getTvVideos(i.id).then(
            (value => localStorage.setItem(`${i.id}`,value["results"][0].key)))); 
    ...
    const indexIncreate = () => {
        if(data) {
            ...
            console.log(totalReults , lastIndex)
    ...
    const onTvClick = (tvId: number) =>{
        history.push(`/tv/onair/${tvId}`)
        getTvVideos(tvId).then(v => {
            setVideoId(v["results"][0].key);
        });
        let old = localStorage.getItem("mylist")
        old = JSON.parse(old || "[]");
        if (old?.includes(tvId+"")){
            setAMylist(true);
        }else{
            setAMylist(false);
        }
    ...
    const goBackClick = () => history.push("/tv");
    ...
    const clickedItem = routeTvMatch?.params.tvId && data?.results.find(
        movie => String(movie.id) === routeTvMatch.params.tvId);
    ...
    return (
        ...
        <Wrapper>
            {isLoading ? 
                ...
                (<>
                    <Banner 
                        bgimage={imagePath(data?.results[0].backdrop_path || "")}>
                        ...
                        <Score>
                            ...
                            <Vote>#1 Rate: {data?.results[0].vote_average}/10</Vote>
                        </Score>
                        <OverView>{data?.results[0].overview}</OverView>
                        <BannerVideo
                        layoutId={data?.results[0].id + ""}
                        onClick={()=>onTvClick(data?.results[0].id || 0)}
                        >See More {'>'}</BannerVideo>
                    </Banner>
                    <Slider>
                    ...
                    <MiniTitle>On the Air {'>'} </MiniTitle>
                    ...
                    <AnimatePresence>
                        ...
                        <InfoTv
                            ...
                            layoutId={routeTvMatch.params.tvId}>
                            <motion.svg
                                onClick={goBackClick}
                            ...
                            {clickedItem && (
                                <>
                                    ...
                                    <InfoTvTitle>
                                        {clickedItem.name}
                                        <InfoTvDetail className="detail">
                                            <h3>First Air: {clickedItem.first_air_date}</h3>
                                            <h5>Score: {clickedItem.vote_average}/10</h5>
                                            <span>Add Mylist ▶︎</span> 
                                        </InfoTvDetail>
                                        <InfoTvAdd  onClick={()=>addMylist(clickedItem.id)}>
                                            {Mylist? (<motion.svg
                                            layoutId="check" 
