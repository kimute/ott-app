...
import{getVideos, getMovie, IGetMovie} from '../api';
import styled from "styled-components";
import { AnimatePresence, motion, useViewportScroll } from "framer-motion";
import { imagePath } from "../utils";
import { useRecoilState } from "recoil";
import { myListstate } from "../atom";
import RecommendSlider from "../Components/Recommend";
...
const MylistBox = styled(motion.div)<{ bgimage: string}>`
    height: 250px;
    background-color: black;
    background-size: cover;
    background-position: center center;
    background-image:linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.0)), url(${(props) => props.bgimage});
    border-radius: 5px;
    cursor: pointer;
...
function Mylist(){
    ...
    useEffect(() => {
        const getItems = localStorage.getItem("mylist");
        if (getItems === "{}" || getItems === null)
          return;
        setMylist(JSON.parse(getItems));
      }, [])
    
    Object.keys(myList).map(i =>console.log(myList[i]));
    const { data, isLoading } = useQuery<IGetMovie>(["movie", "nowPlaying"], getMovie);
    ...
    const routeMovieMatch = useRouteMatch<{movieId: string}>("/mylist/:movieId");
    const { scrollY } = useViewportScroll();
    const increaseIndex = () => {
        if(myList){
            if(leaving)return;
            leavingToggle();
            const totalReults = Object.keys(myList).length;
            const lastIndex = Math.floor(totalReults / offset) ;
    ...
    const filtered = Object.keys(myList).map(i => {
        return data?.results.find(movie => (movie.id === Number(myList[i]) ))
    });

    const clickedItem = routeMovieMatch?.params.movieId && data?.results.find(
        movie => String(movie.id) === routeMovieMatch.params.movieId);

    const goBackClick = (movieId: string) => history.push("/mylist");
        ...
        <Wrapper>
            ...
            (
                ...
                <Btn onClick={increaseIndex}>
                   ...
                   <MylistSlider>
                       <ResultTitle>{filtered.length === 0 ? ' My list > empty':  'My List >'}</ResultTitle>
                       <AnimatePresence initial={false} onExitComplete={toggleClick}>
                           ...
                           key={index}
                           >
                               {filtered.slice(offset*index, offset*index+offset).map((i) =>(<MylistBox
                                    key={i?.id}
                                    layoutId={i?.id +""} 
                                    whileHover="hover"
                                    initial="nomal"
                                    onClick={()=>onMovieClick(Number(i?.id))}
